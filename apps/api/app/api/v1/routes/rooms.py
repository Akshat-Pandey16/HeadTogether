...
from app.models.enums import RoomPurpose
...
async def list_joined_rooms(
    ...
    return await rooms.list_joined(user_id=current_user.id, limit=limit, offset=offset)


@router.get("/owned", response_model=Page[RoomSummary])
async def list_owned_rooms(
    current_user: CurrentUser,
    rooms: RoomServiceDep,
    limit: Annotated[int, Query(ge=1, le=100)] = 50,
    offset: Annotated[int, Query(ge=0)] = 0,
...
async def search_rooms(
    ...
    return await rooms.search_text(actor_id=current_user.id, query=q, limit=limit, offset=offset)
...
async def join_by_code(
    ...
    room = await rooms.join_by_code(code=payload.invite_code, user_id=current_user.id)
    return RoomRead.model_validate(room)
...
async def transfer_ownership(
    ...
    room = await rooms.transfer_ownership(
        room_id=room_id, actor_id=current_user.id, new_owner_id=payload.new_owner_id
    )
    return RoomRead.model_validate(room)
...
async def join_room(
    ...
    await rooms.join_with_location(room_id=room_id, user_id=current_user.id, payload=payload)
    return GenericMessage(message="joined")
